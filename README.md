...
2. **Установите зависимости** и выполните миграции базы данных (используйте Python 3.11, чтобы не требовался Rust для сборки `pydantic-core`):
...
3. Render автоматически найдёт `render.yaml` и предложит создать два сервиса. Blueprint уже фиксирует версию интерпретатора **Python 3.11.8**, что избавляет от ошибок сборки `pydantic-core` в окружении Render.
