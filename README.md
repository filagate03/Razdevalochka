...
2. **Установите зависимости** и выполните миграции базы данных (используйте Python 3.11, чтобы не требовался Rust для сборки `pydantic-core`). Версия зафиксирована в файле `runtime.txt`, поэтому локальная и облачная среды совпадают. При необходимости задайте переменную окружения `PYTHON_VERSION=3.11.8`.
...
3. Render автоматически найдёт `render.yaml` и предложит создать два сервиса. Blueprint уже фиксирует версию интерпретатора **Python 3.11.8** (через переменную окружения `PYTHON_VERSION`), а файл `runtime.txt` страхует ситуацию, если вы запускаете деплой вручную. Это избавляет от ошибок сборки `pydantic-core` в окружении Render.
...
2. **Render:**
   ...
   - Через кнопку **Environment → Bulk Edit** вставьте пары `KEY=VALUE` из вашего `.env`. Не забудьте добавить `PYTHON_VERSION=3.11.8`, если Render не подхватил его автоматически.
